...
XCoinAPI.prototype.xcoinApiCall = async function(endPoint, params) {
  ...
  return await this.request(api_host, 'POST', rgParams, httpHeaders);
}

XCoinAPI.prototype.request = async function(strHost, strMethod, rgParams, httpHeaders) {
  var rgHeaders = {};
  var result = null;
	if(httpHeaders) {
  ...
  // function callbackFunction(error, response, rgResult) {
  ...
  try {
    ...
    if(result.status !== "0000") result = "ERROR";
    return result;
  }
}
